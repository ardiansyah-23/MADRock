  ...
  return (
    <div ref={dropdownRef} className={`relative inline-block ${className}`}>
      <button
        ...
        type="button"
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-100 border border-slate-300 text-xs font-mono font-bold text-slate-800 hover:border-mad-lime transition-all shadow-sm"
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-36 rounded-2xl bg-white border border-slate-200 p-1.5 shadow-xl z-50 animate-fadeIn space-y-1">
          <button
            onClick={() => {
              setLang("id");
              setOpen(false);
