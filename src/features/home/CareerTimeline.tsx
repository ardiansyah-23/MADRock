            ...
            {timeline.map((item, idx) => (
              <ScrollReveal key={idx} delay={0.1 * idx}>
                <div className="relative flex flex-col md:flex-row gap-4 md:gap-8 pl-0 md:pl-20">
                  ...
                  <div className="flex-1 p-5 rounded-2xl bg-white border border-slate-200 hover:border-mad-lime/30 shadow-sm transition-all duration-300 group">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-2">
                      <div>
                        <span className="text-xs font-mono text-slate-500 font-bold uppercase tracking-wider">{item.period}</span>
                        <h4 className="text-slate-900 font-bold text-base font-spartan uppercase mt-0.5">{item.org}</h4>
                        <p className={`text-xs font-mono font-bold ${item.color} mt-0.5`}>{item.role}</p>
                      </div>
                    ...
                    <p className="text-sm text-slate-700 font-medium leading-relaxed border-t border-slate-100 pt-2 mt-2">
                      {item.achievement}
                    </p>
                  </div>
                </div>
